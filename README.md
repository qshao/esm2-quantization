...
runs at all three scales -- 650M (fast dev loop), 3B, and 15B.

Accuracy is measured on the **complete ProteinGym substitution benchmark**: 201
assays, 2.41M variants, at each of the three scales. See
[the full-benchmark section](#full-proteingym-benchmark-201-assays-241m-variants).

## Write-ups

Two documents are built from these results and live in this repo:

| | |
|---|---|
| `preprint/esm2_quantization.pdf` | The paper. 36 pp, 7 figures, 27 tables, 3 algorithms. Build with `preprint/build.sh`, which also packages the arXiv tarball. |
| `report/quantization_report.pdf` | The measurement report. 23 pp, same results, more implementation detail and less argument. Build with `report/build.sh`. |

Both take their figures from `preprint/fig_*.pdf`, regenerated by
...
src/analyze_scales.py       cross-scale Pareto, interaction contrasts, conditioned
                            tail, aggregation conventions, the SNR screen
src/analyze_w8a8.py         per-assay comparison of the two int8_dyn repairs
src/analyze_repeat.py       run-to-run repeatability: primary run vs rep2
src/diag_outliers.py        per-channel activation profile, collapsing assay vs controls
src/figures.py              all seven figures -> preprint/fig_*.pdf
...
slurm/run_outliers.sbatch   activation profiling for the collapse diagnosis
data/proteingym/      3-assay subset (see data/proteingym/README.md)
data/proteingym_v1/   full benchmark: 217 assays + index.json
preprint/             the paper + figures + arXiv packaging
...
The full benchmark is an array job, one task per config. An optional second
positional argument replaces the six-config default -- size the array to match:
...
Anything beginning with `-` is passed through to `run_proteingym.py` instead, so
`... 3B --dms_batch 4` still means what it looks like. 15B wants `--mem=300G
--time=12:00:00`; `int4_wo` alone runs 5.5 h.

Runs are **resumable**: output appends to
`results/proteingym/{model}_{config}.jsonl.gz` and a resubmission skips assays
already written.
...
**The measurement floor is zero.** 8 configs were re-run on different GPUs and
compared per variant: **19,311,304 comparisons, 100% bitwise identical**
(`python src/analyze_repeat.py`). So a +0.0002 difference is a property of the
config, not of the run, and the worst-assay statistic has no run-to-run variance
for a maximum to select from. Determinism removes run variance and nothing else
-- the assay-sampling uncertainty the bootstrap quantifies is untouched.
...
`positions/s` is aggregate throughput -- total masked positions over total
seconds, so it is consistent with the wall-clock column. The median of the 201
per-assay rates is a different number and is given where it matters below.

| config | wall-clock | positions/s | peak RAM median | peak max |
|---|---|---|---|---|
| fp32 | 45.0 min | 15.1 | 11.10 GB | 12.20 GB |
| **bf16** | **7.3 min** | **93.7** | 5.50 GB | 6.05 GB |
| int8_wo | 9.2 min | 74.0 | 2.87 GB | 3.42 GB |
| fp8_dyn | 10.2 min | 66.6 | 2.90 GB | 3.56 GB |
| int8_dyn | 40.0 min | 17.0 | 2.94 GB | 3.51 GB |
| int4_wo | 73.8 min | 9.2 | **1.82 GB** | **2.37 GB** |
...
fallback: 2.87 vs 5.50 GB, 79% of bf16 throughput, max drift 0.016. `int8_dyn` and
...
**2. The workload conflict dissolves at 15B.** DMS speed relative to bf16, under
both aggregation conventions -- `tput` is total positions over total seconds,
`med` is the median of the 201 per-assay rates:

| config | 650M tput | med | 3B tput | med | 15B tput | med |
|---|---|---|---|---|---|---|
| int8_wo | 0.81x | 0.72x | 0.79x | 0.69x | 0.79x | 0.72x |
| **fp8_dyn** | 0.50x | 0.30x | 0.71x | 0.44x | **1.01x** | **0.97x** |
| int8_dyn | 0.13x | 0.07x | 0.18x | 0.10x | 0.24x | 0.20x |
| int4_wo | 0.17x | 0.16x | 0.10x | 0.09x | 0.07x | 0.07x |

The two conventions separate for `fp8_dyn` and for nothing else: its overhead is
fixed per call, so it is worst on the short assays a median weights most
heavily. The convergence holds either way, which is why the claim is safe --
but the numbers are not interchangeable, so state which you mean.
...
|---|---|---|---|---|
| **650M bf16** | 0.4460 | 1.33 | 174.2 | frontier |
| **650M int8_wo** | 0.4457 | **0.73** | 140.6 | frontier |
| **650M int4_wo** | 0.4395 | **0.60** | 29.1 | frontier |
| 3B int4_wo | 0.4435 | 1.82 | 9.2 | dominated by 650M bf16 |
| 3B bf16 | 0.4399 | 5.50 | 93.7 | dominated by 650M bf16 |
| 15B int4_wo | 0.4332 | 7.91 | 2.1 | dominated by 650M bf16 |

(6 of 18 rows; the frontier is the same under either speed convention.)

`3B int4_wo` -- the obvious pick for a small footprint -- uses **more** memory
than `650M bf16`, is less accurate, and is 19x slower. Quantization is still
