...
\caption{\emph{Left:} DMS scoring speed relative to bf16 against model scale.
\cfg{fp8\_dyn} converges on bf16 while the weight-only and INT4 paths do not,
so the ``quantization is pure overhead for DMS'' conclusion is scale-bounded.
\emph{Right:} fp32 mean Spearman against experiment falls monotonically across
a 23-fold parameter increase. No pairwise difference is significant, but there
is no evidence that scale buys variant-effect accuracy, and 15B costs
$5.4\times$ the compute of 3B for the lowest of the three.}
\label{fig:scale}
\end{figure}
