...
so we measured it (Table~\ref{tab:asymbulk}).

\begin{table}[htb]
\centering
\caption{Bulk embedding extraction at 3B on an H200 under
\texttt{max-autotune}, all configurations from a single run so the comparison is
internal. Settings differ from Table~\ref{tab:bulk} (32 sequences at 13.1\%
padding waste rather than 64 at 0\%), so the absolute rates are not comparable
across the two tables; the ratios within this one are. \cfg{int8\_dyn\_asym}
recovers almost all of the fidelity that symmetric scaling loses --- embedding
cosine $0.9906 \to 0.9993$ --- but is $14$\% \emph{slower} than symmetric here,
having been $95$\% faster on DMS.}
\label{tab:asymbulk}
\small
\begin{tabular}{lrrrrr}
\toprule
Config & residues/s & vs bf16 & peak GB & emb.\ cos & DMS $\rho_{\text{fp32}}$ \\
\midrule
\cfg{fp32}            &  5{,}775 & 0.12$\times$ & 19.90 & ---      & --- \\
\cfg{bf16}            & 48{,}459 & 1.00$\times$ &  8.92 & 0.999933 & 0.9998 \\
\cfg{int8\_wo}        & 46{,}434 & 0.96$\times$ &  6.34 & 0.999865 & 0.9998 \\
\cfg{int8\_dyn}       & 49{,}871 & 1.03$\times$ &  5.43 & 0.990609 & 0.9928 \\
\cfg{int8\_dyn\_asym} & 42{,}659 & 0.88$\times$ &  5.58 & 0.999276 & 0.9972 \\
\cfg{fp8\_dyn}        & \textbf{54{,}198} & \textbf{1.12$\times$} &  5.54 & 0.999162 & 0.9988 \\
\bottomrule
\end{tabular}
\end{table}

Two consequences, and the second corrects a recommendation we made on the
strength of the DMS result alone.

The \emph{accuracy} repair transfers: embedding cosine goes from $0.9906$ under
symmetric scaling to $0.9993$ under asymmetric, and DMS $\rho_{\text{fp32}}$
from $0.9928$ to $0.9972$. Whatever symmetric scaling is doing wrong, it is not
specific to the masked-marginals input distribution.

The \emph{speed} effect reverses. Asymmetric is $1.95\times$ faster than
symmetric on DMS and $0.85\times$ its speed on bulk extraction. It is therefore
not the right choice for bulk embedding on hardware without FP8:
\textbf{\cfg{int8\_wo} beats it on both axes that matter there}, being faster
($0.96\times$ against $0.88\times$ bf16) and more faithful (embedding cosine
$0.999865$ against $0.999276$), at $0.76$\,GB more peak memory. Our
recommendation table previously said otherwise, on DMS evidence extrapolated to
a workload where it does not hold.

We were unable to obtain the corresponding A100 measurement --- the partition
was occupied by a multi-day job for the duration of this revision --- so the
ranking above is established on H200 and the A100-specific bulk numbers for the
asymmetric variant remain unmeasured. We state the recommendation for
FP8-less hardware as an inference from the H200 ordering rather than as a
measurement, and mark it as such in Table~\ref{tab:recs}.
...
slower \emph{and} less faithful than \cfg{int8\_wo} there, Table~\ref{tab:asymbulk}).}
\label{tab:recs}
\small
\begin{tabular}{lll}
\toprule
...
Table~\ref{tab:asymbulk}. The A100 partition was occupied by a multi-day job
